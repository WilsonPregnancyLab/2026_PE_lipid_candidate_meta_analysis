...
lipid_ensembl_list <- lipid_ensembl_gencode[!is.na(lipid_ensembl_gencode$transcript_id),] #58433 
...
#Step 3: Construct DESeqDataSet object, removed "instrument" from design because confound perfectly with GSE_number
RNA_Lip_DESeq_chrX_IT <- DESeqDataSetFromMatrix(countData = filt_read_counts_chrX, colData = RNA_Metadata_ex_rem, design = ~ GSE_number + predicted_fetal_sex*disease_group)

#Pre-filtering: removing rows with low gene counts, keeping rows that have at least 10 reads adding across all samples
pre_filter <- rowSums(counts(RNA_Lip_DESeq_chrX_IT)) >=10
RNA_Lip_DESeq_chrX_IT <- RNA_Lip_DESeq_chrX_IT[pre_filter,]

#Set the factor level
RNA_Lip_DESeq_chrX_IT$disease_group <- relevel(RNA_Lip_DESeq_chrX_IT$disease_group, ref = "Control")

#Step 4: Run DESeq
RNA_Lip_DESeq_chrX_IT <- DESeq(RNA_Lip_DESeq_chrX_IT)
## Positive LFC means gene went up more in males, negative LFC means gene went up more in females
RNA_Lip_DESeq_chrX_IT_results <- results(RNA_Lip_DESeq_chrX_IT, name="predicted_fetal_sexM.disease_groupPE")
RNA_Lip_DESeq_chrX_IT_results <- as.data.frame(RNA_Lip_DESeq_chrX_IT_results)
RNA_Lip_DESeq_chrX_IT_results <- merge(RNA_Lip_DESeq_chrX_IT_results, lipid_ensembl_list[, c("gene_id", "gene_symbol", "seqnames")], by.x = "row.names", by.y = "gene_id")
RNA_Lip_DESeq_chrX_IT_results <- RNA_Lip_DESeq_chrX_IT_results[!is.na(RNA_Lip_DESeq_chrX_IT_results$padj),]

RNA_Lip_DESeq_chrX_IT_results$Expression_Status <- "Not_Biologically_Significant" #4753
RNA_Lip_DESeq_chrX_IT_results$Expression_Status[RNA_Lip_DESeq_chrX_IT_results$log2FoldChange > 0.00 & RNA_Lip_DESeq_chrX_IT_results$padj <0.05] <- "Trending_Towards_Increased_RNA_Expression" #0
RNA_Lip_DESeq_chrX_IT_results$Expression_Status[RNA_Lip_DESeq_chrX_IT_results$log2FoldChange < 0.00 & RNA_Lip_DESeq_chrX_IT_results$padj <0.05] <- "Trending_Towards_Decreased_RNA_Expression" #0
RNA_Lip_DESeq_chrX_IT_results$Expression_Status[RNA_Lip_DESeq_chrX_IT_results$log2FoldChange > 1.00 & RNA_Lip_DESeq_chrX_IT_results$padj <0.05] <- "Increased_RNA_Expression" #7
RNA_Lip_DESeq_chrX_IT_results$Expression_Status[RNA_Lip_DESeq_chrX_IT_results$log2FoldChange < -1.00 & RNA_Lip_DESeq_chrX_IT_results$padj <0.05] <- "Decreased_RNA_Expression" #7

write.csv(RNA_Lip_DESeq_chrX_IT_results, "RNA_Lip_DESeq_results_chrX_combined_sex_IT.csv")
...
png("./male_auto_RNAexp_post.png", height = 9, width = 7, units = "in", res = 500)
grid.arrange(male_auto,  nrow = 1)
dev.off()

combined_auto, female_auto, male_auto,

png("./all_autosome_vol_RNAexp_panel.png", height = 9, width = 15, units = "in", res = 500)
