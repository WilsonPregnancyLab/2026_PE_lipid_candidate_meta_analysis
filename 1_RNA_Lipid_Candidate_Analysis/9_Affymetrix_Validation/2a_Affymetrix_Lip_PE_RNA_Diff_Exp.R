...
RNA_seq_sex_stratified <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_Lipid_Candidate/DESeq_rerun/RNA_Lip_DESeq_results_autosomes_IT.csv")
...
#linear_modeling_IT
    ...
    affy_metadata$gestational_age <- as.numeric(as.character(affy_metadata$gestational_age))
    affy_metadata$diagnosis.ch1[affy_metadata$diagnosis.ch1 == "non-PE"] <- "Control"

    PF <- paste(affy_metadata$diagnosis.ch1, affy_metadata$infant.gender.ch1, sep="_")
    PF <- factor(PF, levels=c("Control_M","PE_M","Control_F","PE_F"))
    CONTvsPE_model <- model.matrix(~0 + PF + gestational_age, data = affy_metadata)
    ...
    updated_matrix <- affy_matrix_auto_combined_sex[,colnames(affy_matrix_auto_combined_sex) %in% affy_metadata$file_path]
    ...
    print(sum(tt_CONTvsPE_auto_F$adj.P.Val < 0.05)) #439
    ...
    print(sum(tt_CONTvsPE_auto_M$adj.P.Val < 0.05)) #432
...
# linear_modeling(females, affy_matrix_auto_F, "affy_AvgExpres_auto_F.csv") #203 sig genes
# linear_modeling(males, affy_matrix_auto_M, "affy_AvgExpres_auto_M.csv") #310 sig genes
