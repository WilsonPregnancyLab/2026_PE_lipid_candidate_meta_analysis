...
affymetrix_Lip_PE_final <- readRDS("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_Lipid_Candidate/affymetrix_validation_rerun/affymetrix_Lip_PE_final.rds")
...
RNA_seq_autosomes_combined_sex <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_WG_rerun/RNA_WG_DESeq_results_autosomes_combined_sex.csv") #dim 
RNA_seq_autosomes_combined_sex <- RNA_seq_autosomes_combined_sex[RNA_seq_autosomes_combined_sex$Expression_Status %in% sig, ] #dim 3525, 11

RNA_seq_sex_stratified <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_WG_rerun/RNA_WG_DESeq_results_autosomes_IT.csv")
RNA_seq_autosomes_F <- RNA_seq_sex_stratified[RNA_seq_sex_stratified$comparison == "F_PEvsF_Cont", ]
RNA_seq_autosomes_F <- RNA_seq_autosomes_F[RNA_seq_autosomes_F$Expression_Status %in% sig, ] #dim 1829, 12

RNA_seq_autosomes_M <- RNA_seq_sex_stratified[RNA_seq_sex_stratified$comparison == "M_PEvsM_Cont", ]
RNA_seq_autosomes_M <- RNA_seq_autosomes_M[RNA_seq_autosomes_M$Expression_Status %in% sig, ] #dim 1718, 12

RNA_seq_chrX_F <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_WG_rerun/RNA_WG_DESeq_results_chrX_F.csv") #dim 
RNA_seq_chrX_F <- RNA_seq_chrX_F[RNA_seq_chrX_F$Expression_Status %in% sig, ] #dim 85, 11

RNA_seq_chrX_M <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_WG_rerun/RNA_WG_DESeq_results_chrX_M.csv") #dim 
RNA_seq_chrX_M <- RNA_seq_chrX_M[RNA_seq_chrX_M$Expression_Status %in% sig, ] #dim 22, 11

RNA_seq_chrY_M <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_RNA_WG_rerun/RNA_WG_DESeq_results_chrY_M.csv") #dim 
RNA_seq_chrY_M <- RNA_seq_chrY_M[RNA_seq_chrY_M$Expression_Status %in% sig, ] #dim 0, 11

RNA_seq_autosomes_combined_sex$ENS_ID <- sub("\\..*", "", RNA_seq_autosomes_combined_sex$Row.names)
RNA_seq_autosomes_F$ENS_ID <- sub("\\..*", "", RNA_seq_autosomes_F$gene_id)
RNA_seq_autosomes_M$ENS_ID <- sub("\\..*", "", RNA_seq_autosomes_M$gene_id)
RNA_seq_chrX_F$ENS_ID <- sub("\\..*", "", RNA_seq_chrX_F$Row.names)
RNA_seq_chrX_M$ENS_ID <- sub("\\..*", "", RNA_seq_chrX_M$Row.names)
RNA_seq_chrY_M$ENS_ID <- sub("\\..*", "", RNA_seq_chrY_M$Row.names)
...
sig_probes_auto_combined_sex <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_autosomes_combined_sex$ENS_ID,] #dim 2195, 6
sig_probes_auto_F <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_autosomes_F$ENS_ID,] #dim 1114, 6
sig_probes_auto_M <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_autosomes_M$ENS_ID,] #dim 1203, 6
sig_probes_chrX_F <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_chrX_F$ENS_ID,] #dim 61, 6
sig_probes_chrX_M <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_chrX_M$ENS_ID,] #dim 20, 6
sig_probes_chrY_M <- affy_probe_data[affy_probe_data$ENSEMBL %in% RNA_seq_chrY_M$ENS_ID,] #dim 0, 6
...
affy_matrix_chrY_M <- affy_matrix[rownames(affy_matrix) %in% sig_probes_chrY_M$PROBEID,]
...
expres_table(affy_matrix_chrY_M, PEmetadata_M, Controlmetadata_M)
...
linear_modeling_combined_sex <- function (data, matrix, delta_file){
    ...
    affy_pheno$gestational_age <- as.numeric(as.character(affy_pheno$gestational_age))
    affy_pheno$diagnosis.ch1[affy_pheno$diagnosis.ch1 == "non-PE"] <- "Control"

    PF <- paste(affy_pheno$diagnosis.ch1, affy_pheno$infant.gender.ch1, sep="_")
    PF <- factor(PF, levels=c("Control_M","PE_M","Control_F","PE_F"))
    CONTvsPE_model <- model.matrix(~0 + PF + gestational_age, data = affy_pheno)
    colnames(CONTvsPE_model)[colnames(CONTvsPE_model) == "PFControl_M"] <- "Control_M"
    colnames(CONTvsPE_model)[colnames(CONTvsPE_model) == "PFPE_M"] <- "PE_M"
    colnames(CONTvsPE_model)[colnames(CONTvsPE_model) == "PFControl_F"] <- "Control_F"
    colnames(CONTvsPE_model)[colnames(CONTvsPE_model) == "PFPE_F"] <- "PE_F"
    updated_matrix <- affy_matrix_auto_combined_sex[,colnames(affy_matrix_auto_combined_sex) %in% affy_pheno$file_path]
    ...
    cont_matrix <- makeContrasts(
          ...
          levels = CONTvsPE_fit)
    CONTvsPE_wholefit2 <- contrasts.fit(CONTvsPE_fit, cont_matrix)
    CONTvsPE_wholefit2 <- eBayes(CONTvsPE_wholefit2)
    tt_CONTvsPE_auto_F <- topTable(CONTvsPE_wholefit2, n = Inf, adjust = "fdr", coef = "PEinF")
    print(sum(tt_CONTvsPE_auto_F$adj.P.Val < 0.05)) #1124
    tt_CONTvsPE_auto_F$probes <- rownames(tt_CONTvsPE_auto_F)
    tt_CONTvsPE_auto_F$probes <- as.factor(tt_CONTvsPE_auto_F$probes)
    delta_file <- read.csv("affy_AvgExpres_auto_combined_sex.csv")
    delta_file$probes <- delta_file$ProbePE
    head(delta_file$probes)
    delta_file$probes <- as.factor(delta_file$probes)
    results_F <- merge(merge(tt_CONTvsPE_auto_F, delta_file[, c("deltaExprs", "probes")], by = "probes"), 
            affy_probe_data[,c("PROBEID", "SYMBOL","GENENAME", "CHR", "ENSEMBL", "PMID")], by.x = "probes", by.y = "PROBEID")
    sig_results_F <- results_F[results_F$adj.P.Val < 0.05, ]
    matrix_name <- deparse(substitute(affy_matrix_auto_combined_sex))
    suffix <- sub("affy_matrix_(.*)", "\\1", matrix_name)
    affy_results_name_F <- paste0("affy_results_", suffix, "_F",".csv")
    tt_results_name_F <- paste0("tt_CONTvsPE_", suffix, "_F",".csv")
    sig_results_name_F <- paste0("sig_affy_results_", suffix, "_F",".csv")
    write.csv(results_F, file = affy_results_name_F)
    write.csv(tt_CONTvsPE_auto_F, file = tt_results_name_F)
    write.csv(sig_results_F, file = sig_results_name_F)

    tt_CONTvsPE_auto_M <- topTable(CONTvsPE_wholefit2, n = Inf, adjust = "fdr", coef = "PEinM")
    print(sum(tt_CONTvsPE_auto_M$adj.P.Val < 0.05)) #1045
    tt_CONTvsPE_auto_M$probes <- rownames(tt_CONTvsPE_auto_M)
    tt_CONTvsPE_auto_M$probes <- as.factor(tt_CONTvsPE_auto_M$probes)
    results_M <- merge(merge(tt_CONTvsPE_auto_M, delta_file[, c("deltaExprs", "probes")], by = "probes"), 
            affy_probe_data[,c("PROBEID", "SYMBOL","GENENAME", "CHR", "ENSEMBL", "PMID")], by.x = "probes", by.y = "PROBEID")
    sig_results_M <- results_M[results_M$adj.P.Val < 0.05, ]
    matrix_name <- deparse(substitute(affy_matrix_auto_combined_sex))
    suffix <- sub("affy_matrix_(.*)", "\\1", matrix_name)
    affy_results_name_M <- paste0("affy_results_", suffix, "_M",".csv")
    tt_results_name_M <- paste0("tt_CONTvsPE_", suffix, "_M",".csv")
    sig_results_name_M <- paste0("sig_affy_results_", suffix, "_M",".csv")
    write.csv(results_M, file = affy_results_name_M)
    write.csv(tt_CONTvsPE_auto_M, file = tt_results_name_M)
    write.csv(sig_results_M, file = sig_results_name_M)
...
linear_modeling_combined_sex(affy_metadata, affy_matrix_auto_combined_sex, "affy_AvgExpres_auto_combined_sex.csv") #1443 sig genes
#linear_modeling(females, affy_matrix_auto_F, "affy_AvgExpres_auto_F.csv") #520 sig genes
#linear_modeling(males, affy_matrix_auto_M, "affy_AvgExpres_auto_M.csv") #711 sig genes
linear_modeling(females, affy_matrix_chrX_F, "affy_AvgExpres_chrX_F.csv") #27 sig genes
linear_modeling(males, affy_matrix_chrX_M, "affy_AvgExpres_chrX_M.csv") #18 sig genes
#linear_modeling(males, affy_matrix_chrY_M, "affy_AvgExpres_chrY_M.csv") #0/10 sig genes
...
affy_results_auto_F <- read.csv("affy_results_auto_combined_sex_F.csv")
affy_results_auto_M <- read.csv("affy_results_auto_combined_sex_M.csv")
affy_results_chrX_F <- read.csv("affy_results_chrX_F.csv")
affy_results_chrX_M <- read.csv("affy_results_chrX_M.csv")
#affy_results_chrY_M <- read.csv("affy_results_chrY_M.csv")
...
# affy_results_chrY_M$Expression_Status <- "Not_Biologically_Significant"
# affy_results_chrY_M$Expression_Status[affy_results_chrY_M$logFC > 0.00 & affy_results_chrY_M$adj.P.Val <0.05] <- "Trending Towards Increased Expression"
# affy_results_chrY_M$Expression_Status[affy_results_chrY_M$logFC < 0.00 & affy_results_chrY_M$adj.P.Val <0.05] <- "Trending Towards Decreased Expression"
# affy_results_chrY_M$Expression_Status[affy_results_chrY_M$logFC > 1.00 & affy_results_chrY_M$adj.P.Val <0.05] <- "Increased Expression"
# affy_results_chrY_M$Expression_Status[affy_results_chrY_M$logFC < -1.00 & affy_results_chrY_M$adj.P.Val <0.05] <- "Decreased Expression"
...
#male_chrY_sig <- subset(affy_results_chrY_M[affy_results_chrY_M$adj.P.Val <0.05,]) #
...
#affy_auto_combined_sex$siglabel <- ifelse(affy_auto_combined_sex$probes %in% combined_sex_auto_sig$probes, affy_auto_combined_sex$SYMBOL, NA)
...
affy_results_chrX_F$siglabel <- ifelse(affy_results_chrX_F$Expression_Status %in% c("Increased Expression", "Decreased Expression"), affy_results_chrX_F$SYMBOL, NA)
...
affy_results_chrX_M$siglabel <- ifelse(affy_results_chrX_M$Expression_Status %in% c("Increased Expression", "Decreased Expression"), affy_results_chrX_M$SYMBOL, NA)
...
# affy_results_chrY_M$siglabel <- ifelse(affy_results_chrY_M$Expression_Status %in% c("Increased Expression", "Decreased Expression"), affy_results_chrY_M$SYMBOL, NA)
# male_Y <- ggplot(data = affy_results_chrY_M, aes(x = logFC, y = -log10(adj.P.Val), col = Expression_Status)) +
#   geom_point(shape = 19, alpha = 0.4, size = 3) +
#   theme_bw() +
#   ylab(" ") +
#   theme(axis.text = element_text(size = 14),
...
png("./X_vol_RNA_affy_panel.png", height = 9, width = 20, units = "in", res = 300)
