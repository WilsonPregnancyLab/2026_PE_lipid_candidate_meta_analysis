...
BiocManager::install(c("missMDA","IlluminaHumanMethylation450kanno.ilmn12.hg19","IlluminaHumanMethylation450kmanifest","minfi","magrittr","wateRmelon","gridExtra","ggrepel"))

library(missMDA) #version 1.20
...
probeInfo <- as.data.frame(cbind(IlluminaHumanMethylation450kanno.ilmn12.hg19::Locations, IlluminaHumanMethylation450kanno.ilmn12.hg19::Other, IlluminaHumanMethylation450kanno.ilmn12.hg19::Manifest)) 
...
metadata <- read.csv("./Metadata_Sheet_lipid_preeclampsia_excluded_removed.csv")
metadata$exclude[is.na(metadata$exclude)] <- "keep"
metadata <- subset(metadata, exclude != "exclude")
...
number_bad_detp <- print(sum(rowSums(bad_detp)>=(ncol(placmet_noNorm))*0.05)) #556
...
price_anno <- read.delim("./GPL16304-47833.txt",header=TRUE)
...
library(RCurl) #version 1.98.1.14
...
noNorm_pca_df$GSE_number <- metadata$GSE_number[match(rownames(noNorm_pca_df), metadata$Sample_Name)]
noNorm_pca_df$Fetal_Sex <- metadata$Fetal_Sex[match(rownames(noNorm_pca_df), metadata$Sample_Name)]
noNorm_pca_df$pathology_group <- metadata$pathology_group[match(rownames(noNorm_pca_df), metadata$Sample_Name)]
noNorm_pca_df <- noNorm_pca_df[!is.na(noNorm_pca_df$GSE_number), ]
...
adjFunnorm_pca_df$GSE_number <- metadata$GSE_number[match(rownames(adjFunnorm_pca_df), metadata$Sample_Name)]
adjFunnorm_pca_df$Fetal_Sex <- metadata$Fetal_Sex[match(rownames(adjFunnorm_pca_df), metadata$Sample_Name)]
adjFunnorm_pca_df$pathology_group <- metadata$pathology_group[match(rownames(adjFunnorm_pca_df), metadata$Sample_Name)]
adjFunnorm_pca_df <- adjFunnorm_pca_df[!is.na(adjFunnorm_pca_df$GSE_number), ]
...
PCA_noNorm <- ggplot(noNorm_pca_df, aes(x = PC1, y = PC2, color = GSE_number, fill = GSE_number)) +
  ...
  scale_x_continuous(breaks = seq(-1000, 1000, by = 250), limits = c(-1000, 1000)) +
  scale_y_continuous(breaks = seq(-1000, 1000, by = 250), limits = c(-1000, 1000)) +
  ...
  labs(title = "PCA of Batch Uncorrected DNAm Data - Study")
...
PCA_adjFunnorm <- ggplot(adjFunnorm_pca_df, aes(x = PC1, y = PC2, color = GSE_number, fill = GSE_number)) +
  ...
  xlab("PC1 (12.2%)") + 
  stat_ellipse(geom="polygon", level = 0.95, alpha = 0.25) +
  labs(title = "PCA of Batch Corrected DNAm Data - Study")

png("./PCA_Bef_Aft_Norm_1.png", height = 8, width = 20, units = "in", res = 300)
grid.arrange(PCA_noNorm, PCA_adjFunnorm, nrow = 1)
dev.off()

png("./PCA_noNorm_1.png", height = 9, width = 15, units = "in", res = 300)
grid.arrange(PCA_noNorm, nrow = 1)
dev.off()

png("./PCA_adjFunnorm_1.png", height = 9, width = 15, units = "in", res = 300)
...
PCA_noNorm_pg <- ggplot(noNorm_pca_df, aes(x = PC1, y = PC2, color = pathology_group, fill = pathology_group)) +
  ...
  labs(title = "PCA Before Functional Normalization - Pathology Group")
...
PCA_adjFunnorm_pg <- ggplot(adjFunnorm_pca_df, aes(x = PC1, y = PC2, color = pathology_group, fill = pathology_group)) +
  ...
  xlab("PC1 (12.2%)") + 
  stat_ellipse(geom="polygon", level = 0.95, alpha = 0.2) +
  labs(title = "PCA After Functional Normalization - Pathology Group")
...
PCA_noNorm_pg <- ggplot(noNorm_pca_df, aes(x = PC1, y = PC2, color = Fetal_Sex, fill = Fetal_Sex)) +
  geom_point(size = 3) +
  theme_classic() +
  theme(plot.title = element_text(size = 24, face = "bold", hjust = 0.5),
        axis.text = element_text(size = 14),
  ...
  xlab("PC1 (32.6%)") + 
  stat_ellipse(geom="polygon", level = 0.95, alpha = 0.2) +
  labs(title = "PCA Before Functional Normalization - Fetal Sex")

PCA_adjFunnorm_pg <- ggplot(adjFunnorm_pca_df, aes(x = PC1, y = PC2, color = Fetal_Sex, fill = Fetal_Sex)) +
  geom_point(size = 3) +
  theme_classic() +
  theme(plot.title = element_text(size = 24, face = "bold", hjust = 0.5),
        axis.text = element_text(size = 14),
  ...
  xlab("PC1 (12.2%)") + 
  stat_ellipse(geom="polygon", level = 0.95, alpha = 0.2) +
  labs(title = "PCA After Functional Normalization - Fetal Sex")

png("./PCA_Bef_Aft_Norm_fetal_sex.png", height = 9, width = 30, units = "in", res = 300)
grid.arrange(PCA_noNorm_pg, PCA_adjFunnorm_pg, nrow = 1)
dev.off()

png("./PCA_noNorm_fetal_sex.png", height = 9, width = 15, units = "in", res = 300)
grid.arrange(PCA_noNorm_pg, nrow = 1)
dev.off()

png("./PCA_adjFunnorm_fetal_sex.png", height = 9, width = 15, units = "in", res = 300)
grid.arrange(PCA_adjFunnorm_pg, nrow = 1)
dev.off()
