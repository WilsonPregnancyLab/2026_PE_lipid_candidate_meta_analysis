...
all_probes <- annotation_price38[!duplicated(annotation_price38$CpG_ID),]
# price_anno_probes <- read.csv("/workspace/lab/wilsonslab/eyerk/2025_Whole_Genome_PlacMet/price_anno_probes.csv")
chrXprobes <- subset(all_probes, all_probes$chr == "chrX") #10390 probes
chrYprobes <- subset(all_probes, all_probes$chr == "chrY") #380 probes
...
placmet_adjFunnorm_filtfun_F <- placmet_adjFunnorm_filtbetas[, females$Sample_Name] #424750 83
placmet_adjFunnorm_filtfun_M <- placmet_adjFunnorm_filtbetas[, males$Sample_Name] #424750 89

# Combined Autosomes
combined_auto_wg_beta <- placmet_adjFunnorm_filtbetas[!rownames(placmet_adjFunnorm_filtbetas) %in% c(chrXprobes$CpG_ID, chrYprobes$CpG_ID), ] #dim 415365 172
# Female Autosomes
F_auto_wg_beta <- placmet_adjFunnorm_filtfun_F[!rownames(placmet_adjFunnorm_filtfun_F) %in% c(chrXprobes$CpG_ID, chrYprobes$CpG_ID), ] #dim 415365 83
F_X_wg_beta <- placmet_adjFunnorm_filtfun_F[rownames(placmet_adjFunnorm_filtfun_F) %in% c(chrXprobes$CpG_ID), ] #dim 9143 83

# Male Autosomes
M_auto_wg_beta <- placmet_adjFunnorm_filtfun_M[!rownames(placmet_adjFunnorm_filtfun_M) %in% c(chrXprobes$CpG_ID, chrYprobes$CpG_ID), ] #dim 415365 89
M_X_wg_beta <- placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% c(chrXprobes$CpG_ID), ] #dim 9143 89
M_Y_wg_beta <- placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% c(chrYprobes$CpG_ID), ] #dim 242 89
...
#8 ind sig probes, no genes annotated to sig CpGs
