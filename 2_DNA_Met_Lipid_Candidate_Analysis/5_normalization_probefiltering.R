...
number_bad_detp <- print(sum(rowSums(bad_detp)>=(ncol(combined_RGset))*0.05)) #561
# missing betas >= 5% of samples 
avgbeta <- getBeta(combined_RGset)
bad_beta <- is.na(avgbeta)
number_bad_beta <- print(sum(rowSums(bad_beta)>=(ncol(combined_RGset))*0.05)) #16
...
number_badprobes <- print(sum(rowSums(badProbes1, na.rm = TRUE))) #562
