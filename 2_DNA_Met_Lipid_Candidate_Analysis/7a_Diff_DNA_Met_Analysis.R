...
gtf_file <- "/workspace/lab/wilsonslab/datalake-wilsonslab/PE_Lipid_Meta-analysis/2025_RNA_Lipid_Candidate/genome_mapping/gencode.v48.primary_assembly.annotation.gtf"
gtf_data <- import(gtf_file)
gtf_df <- as.data.frame(gtf_data)
#gtf_df$gene_id <- sub("\\..*", "", gtf_df$gene_id)
gtf_df_dups_rem <- gtf_df[!duplicated(gtf_df$gene_id),]

lipid_gene_list <- read.csv("/workspace/lab/wilsonslab/datalake-wilsonslab/PE_Lipid_Meta-analysis/2025_RNA_Lipid_Candidate/DESeq_analysis/lipid_genes_unique_P.csv")
...
annotation_price38 <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_DNAm_Lipid_Candidate/Candidate_DNAm_Annotations/updated_full_annotation_region_vs_price38.csv")

# Subset annotation_price38 file so that it only includes my lipid genes
lipid_candidate_probes_full <- annotation_price38[annotation_price38$gene %in% lipid_ensembl_list$gene_symbol, ] #146342 probes
lipid_candidate_probes <- lipid_candidate_probes_full[!duplicated(lipid_candidate_probes_full$CpG_ID),] #138524 probes
...
placmet_PE_all_autosomes <- as.data.frame(placmet_adjFunnorm_filtbetas[!rownames(placmet_adjFunnorm_filtbetas) %in% c(chrXprobes$CpG_ID, chrYprobes$CpG_ID), PEmetadata_all$Sample_Name]) #dim 122609 100
placmet_CONT_all_autosomes <- as.data.frame(placmet_adjFunnorm_filtbetas[!rownames(placmet_adjFunnorm_filtbetas) %in% c(chrXprobes$CpG_ID, chrYprobes$CpG_ID), Controlmetadata_all$Sample_Name]) #dim 122609 72
...
#Autosomes
placmet_PE_M_autosomes <- as.data.frame(placmet_adjFunnorm_filtfun_M[!rownames(placmet_adjFunnorm_filtfun_M) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID), PEmetadata_M$Sample_Name])#dim: 122609, 52
placmet_CONT_M_autosomes <- as.data.frame(placmet_adjFunnorm_filtfun_M[!rownames(placmet_adjFunnorm_filtfun_M) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID), Controlmetadata_M$Sample_Name]) #dim: 122609, 37
...
#Male X Chromosome 
placmet_PE_M_X <-  as.data.frame(placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% chrXprobes$CpG_ID, PEmetadata_M$Sample_Name]) #dim: 2360, 52
placmet_CONT_M_X <- as.data.frame(placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% chrXprobes$CpG_ID, Controlmetadata_M$Sample_Name]) #dim: 2360, 37
...
# placmet_PE_M_Y <-  as.data.frame(placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% chrYprobes$CpG_ID, PEmetadata_M$Sample_Name]) #dim: 5, 52 
# placmet_CONT_M_Y <- as.data.frame(placmet_adjFunnorm_filtfun_M[rownames(placmet_adjFunnorm_filtfun_M) %in% chrYprobes$CpG_ID, Controlmetadata_M$Sample_Name]) #dim: 5, 37
...
#Female Autosomes
placmet_PE_F_autosomes <-  as.data.frame(placmet_adjFunnorm_filtfun_F[!rownames(placmet_adjFunnorm_filtfun_F) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID), PEmetadata_F$Sample_Name]) #dim: 122609, 48
placmet_CONT_F_autosomes <- as.data.frame(placmet_adjFunnorm_filtfun_F[!rownames(placmet_adjFunnorm_filtfun_F) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID), Controlmetadata_F$Sample_Name]) #dim: 122609, 35
...
#Female X Chromosome 
placmet_PE_F_X <-  as.data.frame(placmet_adjFunnorm_filtfun_F[rownames(placmet_adjFunnorm_filtfun_F) %in% chrXprobes$CpG_ID, PEmetadata_F$Sample_Name])  #dim: 2360, 48
placmet_CONT_F_X <- as.data.frame(placmet_adjFunnorm_filtfun_F[rownames(placmet_adjFunnorm_filtfun_F) %in% chrXprobes$CpG_ID, Controlmetadata_F$Sample_Name])  #dim: 2360, 35
...
#Whole population 
placmet_adjFunnorm_MVal <- beta2m(placmet_adjFunnorm_filtbetas) #dim 124969, 172
write.csv(placmet_adjFunnorm_MVal, "./placmet_adjFunnorm_MVal.csv")
#whole population Mvalues - Autosomes
Mval_whole_auto <- placmet_adjFunnorm_MVal[!rownames(placmet_adjFunnorm_MVal) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID),] #122609, 172
...
#male autosomes
Mval_male_auto <- Mval_male[!rownames(Mval_male) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID),] #122609, 89
write.csv(Mval_male_auto, "./Mval_male_auto.csv")
#male XChr
Mval_male_X <- Mval_male[rownames(Mval_male) %in% chrXprobes$CpG_ID,] #2360, 89
write.csv(Mval_male_X, "./Mval_male_X.csv")
#male YChr
Mval_male_Y <- Mval_male[rownames(Mval_male) %in% chrYprobes$CpG_ID,] #0, 89
...
#female autosomes 
Mval_female_auto <- Mval_female[!rownames(Mval_female) %in% c(chrXprobes$CpG_ID,chrYprobes$CpG_ID),] #122609, 83
write.csv(Mval_female_auto, "./Mval_female_auto.csv")
#female XChr 
Mval_female_X <- Mval_female[rownames(Mval_female) %in% chrXprobes$CpG_ID,] #2360 83
...
CONTvsPE_wholefit_auto <- eBayes(CONTvsPE_wholefit_auto) #dim 122609, 7
...
write.csv(tt_CONTvsPE_whole_auto, "./tt_CONTvsPE_whole_auto_study_rerun.csv")
write.csv(placmet_wholepop_auto, "./placmet_wholepop_auto_rerun.csv") #129090 15
auto_sig <- subset(placmet_wholepop_auto[placmet_wholepop_auto$adj.P.Val <0.05,]) #0
...
print(sum(tt_CONTvsPE_wholefit2_auto_M$adj.P.Val < 0.05)) #0
...
write.csv(placmet_F_IT_auto, "./placmet_F_IT_auto_rerun.csv") #129090 15
...
write.csv(placmet_M_IT_auto, "./placmet_M_IT_auto_rerun.csv") #129090 15
auto_sig <- subset(placmet_M_IT_auto[placmet_M_IT_auto$adj.P.Val <0.05,]) #0


#male autosomes - sex-stratified
...
print(sum(tt_CONTvsPE_M_all_auto$adj.P.Val < 0.05)) #16
...
placmet_M_fulldata_auto <- merge(merge(tt_CONTvsPE_M_all_auto, placmet_MaleAvgbetas_autosomes[, c("deltaB","probes")], by = "probes"),
                               annotation_price38[,c("gene", "probes", "chr","position", "region_overlap", "Closest_TSS_gene_name", "Closest_TSS_Transcript", "Closest_TSS_Pos", "Distance_Closest_TSS", "gene_id", "overlap")], by = "probes")
write.csv(tt_CONTvsPE_M_all_auto, "./tt_CONTvsPE_M_all_auto_study_rerun.csv")
write.csv(placmet_M_fulldata_auto, "./placmet_M_fulldata_auto_rerun.csv")
maleauto_sig <- subset(placmet_M_fulldata_auto[placmet_M_fulldata_auto$adj.P.Val <0.05,]) #6 15 (4 unique probes but annotate to 6 different genes)
...
placmet_M_fulldata_X <- merge(merge(tt_CONTvsPE_M_X, placmet_MaleAvgbetas_X[, c("deltaB","probes")], by = "probes"),
                               annotation_price38[,c("gene", "probes", "chr","position", "region_overlap", "Closest_TSS_gene_name", "Closest_TSS_Transcript", "Closest_TSS_Pos", "Distance_Closest_TSS", "gene_id", "overlap")], by = "probes")
write.csv(tt_CONTvsPE_M_X, "./tt_CONTvsPE_M_X_study_rerun.csv")
write.csv(placmet_M_fulldata_X, "./placmet_M_fulldata_X_rerun.csv")
...
#                                annotation_price38[,c("gene", "probes", "chr","position", "region_overlap", "Closest_TSS_gene_name", "Closest_TSS_Transcript", "Closest_TSS_Pos", "Distance_Closest_TSS", "gene_id", "overlap")], by = "probes")
# write.csv(tt_CONTvsPE_M_Y, "./tt_CONTvsPE_M_Y_study_rerun.csv")
# write.csv(placmet_M_fulldata_Y, "./placmet_M_fulldata_Y_rerun.csv")

#Female Autosomes - sex-stratified 
...
print(sum(tt_CONTvsPE_F_all_auto$adj.P.Val < 0.05)) # 1
...
placmet_F_fulldata_auto <- merge(merge(tt_CONTvsPE_F_all_auto, placmet_FemaleAvgbetas_autosomes[, c("deltaB","probes")], by = "probes"),
                               annotation_price38[,c("gene", "probes", "chr","position", "region_overlap", "Closest_TSS_gene_name", "Closest_TSS_Transcript", "Closest_TSS_Pos", "Distance_Closest_TSS", "gene_id", "overlap")], by = "probes")
write.csv(tt_CONTvsPE_F_all_auto, "./tt_CONTvsPE_F_all_auto_study_rerun.csv")
write.csv(placmet_F_fulldata_auto, "./placmet_F_fulldata_auto_rerun.csv")
femaleauto_sig <- subset(placmet_F_fulldata_auto[placmet_F_fulldata_auto$adj.P.Val <0.05,]) #23 15 (16 unique probes but annotate to 23 different genes)
...
placmet_F_fulldata_X <- merge(merge(tt_CONTvsPE_F_X, placmet_FemaleAvgbetas_X[, c("deltaB","probes")], by = "probes"),
                               annotation_price38[,c("gene", "probes", "chr","position", "region_overlap", "Closest_TSS_gene_name", "Closest_TSS_Transcript", "Closest_TSS_Pos", "Distance_Closest_TSS", "gene_id", "overlap")], by = "probes")
write.csv(tt_CONTvsPE_F_X, "./tt_CONTvsPE_F_X_study_rerun.csv")
write.csv(placmet_F_fulldata_X, "./placmet_F_fulldata_X_rerun.csv")
...
placmet_wholepop_auto <- read.csv ("placmet_wholepop_auto_rerun.csv")
placmet_M_fulldata_auto <- read.csv("placmet_M_IT_auto_rerun.csv")
placmet_M_fulldata_X <- read.csv("placmet_M_fulldata_X_rerun.csv")
placmet_F_fulldata_auto <- read.csv("placmet_F_IT_auto_rerun.csv")
placmet_F_fulldata_X <- read.csv("placmet_F_fulldata_X_rerun.csv")
...
colnames(plot_wholepop_auto)[colnames(plot_wholepop_auto) == "diffmethylation"] <- "diffmethylation_whole"
colnames(plot_F_fulldata_auto)[colnames(plot_F_fulldata_auto) == "diffmethylation"] <- "diffmethylation_F"
colnames(plot_M_fulldata_auto)[colnames(plot_M_fulldata_auto) == "diffmethylation"] <- "diffmethylation_M"

# rename adj.P.Val columns for better identification by group
colnames(plot_wholepop_auto)[colnames(plot_wholepop_auto) == "adj.P.Val"] <- "adj.P.Val_whole"
colnames(plot_F_fulldata_auto)[colnames(plot_F_fulldata_auto) == "adj.P.Val"] <- "adj.P.Val_F"
colnames(plot_M_fulldata_auto)[colnames(plot_M_fulldata_auto) == "adj.P.Val"] <- "adj.P.Val_M"

# rename deltaB columns for better identification by group
colnames(plot_wholepop_auto)[colnames(plot_wholepop_auto) == "deltaB"] <- "deltaB_whole"
colnames(plot_F_fulldata_auto)[colnames(plot_F_fulldata_auto) == "deltaB"] <- "deltaB_F"
colnames(plot_M_fulldata_auto)[colnames(plot_M_fulldata_auto) == "deltaB"] <- "deltaB_M"
...
wholeauto_sig <- subset(plot_wholepop_auto[plot_wholepop_auto$adj.P.Val_whole <0.05,])
femaleauto_sig <- subset(plot_F_fulldata_auto[plot_F_fulldata_auto$adj.P.Val_F <0.05,]) #2
maleauto_sig <- subset(plot_M_fulldata_auto[plot_M_fulldata_auto$adj.P.Val_M <0.05,]) #16
wholeauto_bio_sig<- subset(plot_wholepop_auto[plot_wholepop_auto$adj.P.Val_whole <0.05 & (plot_wholepop_auto$deltaB_whole < -0.05 | plot_wholepop_auto$deltaB_whole > 0.05), ]) #0
femaleauto_bio_sig <- subset(plot_F_fulldata_auto[plot_F_fulldata_auto$adj.P.Val_F <0.05 & (plot_F_fulldata_auto$deltaB_F < -0.05 | plot_F_fulldata_auto$deltaB_F > 0.05), ]) #2
maleauto_bio_sig <- subset(plot_M_fulldata_auto[plot_M_fulldata_auto$adj.P.Val_M <0.05 & (plot_M_fulldata_auto$deltaB_M < -0.05 | plot_M_fulldata_auto$deltaB_M > 0.05), ]) #10
...
plot_wholepop_auto$siglabel <- ifelse(plot_wholepop_auto$probes %in% wholeauto_bio_sig$probes, plot_wholepop_auto$gene, NA)
wholepop_auto <- ggplot(data = plot_wholepop_auto, aes(x = deltaB, y = -log10(adj.P.Val), col = diffmethylation)) + 
geom_point(shape = 19, alpha = 0.7, size = 5) + 
  theme_bw() +
  labs(title = "Combined-Sex Autosomal\nLipid-Candidate Differential DNAm") +
...
plot_M_fulldata_auto$siglabel <- ifelse(plot_M_fulldata_auto$probes %in% maleauto_bio_sig$probes, plot_M_fulldata_auto$gene, NA)
male_auto <- ggplot(data = plot_M_fulldata_auto, aes(x = deltaB, y = -log10(adj.P.Val), col = diffmethylation)) + 
  ylab(" ") +
  geom_point(shape = 19, alpha = 0.5, size = 5) + 
  theme_bw() +
...
plot_F_fulldata_auto$siglabel <- ifelse(plot_F_fulldata_auto$probes %in% femaleauto_bio_sig$probes, plot_F_fulldata_auto$gene, NA)
female_auto <- ggplot(data = plot_F_fulldata_auto, aes(x = deltaB, y = -log10(adj.P.Val), col = diffmethylation)) + 
   geom_point(shape = 19, alpha = 0.5, size = 5) + 
  theme_bw() +
...
png("./autosome_panel_vol_met.png", height = 9, width = 15, units = "in", res = 500)
grid.arrange(wholepop_auto, female_auto, male_auto, nrow = 1)
dev.off()

#plots of X chromosome 
male_X <- ggplot(data = plot_M_fulldata_X, aes(x = deltaB, y = -log10(adj.P.Val), col = diffmethylation)) + 
  geom_vline(xintercept = c(-0.05,0.05), col = "black", linetype = "dashed", linewidth = 0.75) +
  geom_hline(yintercept = c(-log10(0.05)), col = "black", linetype = "dashed", linewidth = 0.75) +
  ylab(" ") +
  geom_point(shape = 19, alpha = 0.5, size = 5) + 
  ...
  xlab("Delta Beta") +
  scale_y_continuous(breaks = seq(0, 2.1, by = 0.5), limits = c(0, 2.1)) +
  scale_x_continuous(breaks = seq(-0.25, 0.25, by = 0.1), limits = c(-0.25, 0.25))  +
  scale_color_manual(values = c("grey", "grey"), 
                     labels = c("Not Biologically Significant"),
                     guide = "none"
)

female_X <- ggplot(data = plot_F_fulldata_X, aes(x = deltaB, y = -log10(adj.P.Val), col = diffmethylation)) + 
  geom_vline(xintercept = c(-0.05,0.05), col = "black", linetype = "dashed", linewidth = 0.75) +
  geom_hline(yintercept = c(-log10(0.05)), col = "black", linetype = "dashed", linewidth = 0.75) +
  ylab("-log10(FDR)") +
  geom_point(shape = 19, alpha = 0.5, size = 5) + 
