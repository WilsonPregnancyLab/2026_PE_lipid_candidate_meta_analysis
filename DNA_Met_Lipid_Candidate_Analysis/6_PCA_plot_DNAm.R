...
nonvarplac_anno <- read.csv("/workspace/lab/wilsonslab/eyerk/PE_Lipid_Meta-analysis/2025_DNAm_Lipid_Candidate/Candidate_Diff_DNAm_Analysis_2025/Invariant_Placenta_CpGs.csv", sep = ",") 
...
PCA_noNorm <- ggplot(noNorm_pca_df, aes(x = PC1, y = PC2, color = GSE_number, fill = GSE_number)) +
  ...
  labs(title = "PCA Before Functional Normalization - Study")
...
PCA_adjFunnorm <- ggplot(adjFunnorm_pca_df, aes(x = PC1, y = PC2, color = GSE_number, fill = GSE_number)) +
  ...
  xlab("PC1 (12.2%)") + 
  stat_ellipse(geom="polygon", level = 0.95, alpha = 0.2) +
  labs(title = "PCA After Functional Normalization - Study")
