...
# Goal: add the NCBI annotations to the lipid genes (First, I'm going to do it for all of the lipid genes I tested - in whole_pop, M and F, then only for the significant ones in each group)
...
# Goal: run precision-recall enrichment analysis in ermineJ to identify overrepresented lipid pathways in whole population, male, and female datasets
# Parameters: scoreColumn = column number in dataset containing p-vals, logTrans = whether p-vals were log-transformed, bigIsBetter = big p-vals are more significant, iterations = number of times software performs random permutation test
